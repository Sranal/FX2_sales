...
//         項目を「0」にしてください。（表示したい場合は「1」にします。）
 ...
 cdt_limit    = '2025/09/07 23:59:59'; // ページを表示する最終期日
 cdt_refresh  = 100;		      // カウンターの更新間隔（1/1000秒単位）
 cdt_msecon   = 1;		      // 1/1000秒単位のカウンタの表示有無
 cdt_message  = '終了まであと・・・<br class="sp_disp">';// カウントダウン中のメッセージ
 cdt_timeout  = '終了準備中';// カウントダウン終了時のメッセージ
 cdt_waittime = 0;		      // カウントダウン終了時の転送待ち秒数
...
//          項目を「0」にしてください。（表示したい場合は「1」にします。）
  ...
  cdl_keyname  = 'fxkdaeesos'; // サイト識別ID。サイト毎に割り当てる。
  cdl_expireday   = '4';   			// ●日後までのカウントダウン
  cdl_expierehour = '24'; 			// ▲時までのカウントダウン(24H)
  cdl_refresh  = 100;		      // カウンターの更新間隔（1/1000秒単位）
  cdl_msecon   = 1;		      // 1/1000秒単位のカウンタの表示有無
  cdl_message  = '終了まであと・・・<br class="sp_disp">';// カウントダウン中のメッセージ
  cdl_timeout  = '終了準備中';// カウントダウン終了時のメッセージ
  cdl_waittime = 0;		      // カウントダウン終了時の転送待ち秒数
  cdl_location = ''; // カウントダウン終了時の転送先（../fin）等urlをいれてください。空の場合は文言表示になります。
...
//         にしてください。（表示したい場合は「1」にします。）
 ...
 lmo_keyname  = '20250312';             // サイト識別ID。サイト毎に割り当てる。
 lmo_expire   = 3;		      // ページ表示期間（単位：時間）
 lmo_refresh  = 100;		      // カウンターの更新間隔（単位：1/1000秒）
 lmo_msecon   = 0;		      // 1/1000秒単位のカウンタの表示有無
 lmo_message  = '';              // カウントダウン中のメッセージ
 lmo_timeout  = '';     // カウントダウン終了時のメッセージ
...
function cmo_CountDown( cmo_limit, cmo_message, cmo_timeout, cmo_msecon ){
	...
	if ( l_time > 0){ // 残り時間の評価
		...
		// 文字列表示用データ生成
		cmo_text = cmo_message + l_day + "日 " + '<span class="hm_area">' + l_hour + "時間 " + l_min + "分 " + '</span>'
